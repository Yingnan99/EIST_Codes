...
//	}
	...
	public void produceR2Async(InboxClient client) {
		var request = createHttpEntity(Droids.R2);
		CompletableFuture.supplyAsync(() -> rest.postForObject(BASE_URL + R_2, request, String.class))
				.thenAccept(result -> client.droidReadyR2(result))
				.join();
	...
	public void produce3POAsync(InboxClient client) {
		var request = createHttpEntity(Droids.THREE_PO);
		CompletableFuture.supplyAsync(() -> rest.postForObject(BASE_URL + THREE_PO, request, String.class))
				.thenAccept(result -> client.droidReady3PO(result))
				.join();
